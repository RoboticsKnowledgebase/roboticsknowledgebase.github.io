...
- Containers: These are the runnable instances of an image. If an image is a recipe, the container is the resulting dish.
...
- Mount Host Directory to Container: Mounting a host directory allows the container to access and persist data on the host machine.
...
Files created within a container's shared volume are often owned by the root user, which may restrict access on the host system.
...
The `-f` flag allows specifying an alternative Docker Compose file. If omitted, it defaults to `docker-compose.yml`.
...
Similar to the previous section, a container can be accessed from another terminal using:

```bash
$ docker compose exec <docker_name> sh
```

In this command, `<docker_name>` corresponds to the name specified in the `docker-compose.yml` file, and `sh` specifies the command to execute (in this case, opening a shell).
